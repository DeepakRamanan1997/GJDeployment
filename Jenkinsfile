        ...
        stage('Deploy') {
            steps {
                ...
                script {
                    ...
                    sh "curl -v -u ${admin}:${admin} --upload-file ${/var/lib/jenkins/workspace/package/target/addressbook.war} ${http://3.95.7.189:9090/manager/text}/deploy?path=/Deepak&update=true"
                    ...
                    sh "curl -v -u ${admin}:${admin} ${http://3.95.7.189:9090/manager/text}/reload?path=/Deepak"
                }
            }
        }
    }
